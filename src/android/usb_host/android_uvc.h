...
typedef struct usbhost_uvc_interface_handle{
    std::shared_ptr<librealuvc::usb_host::device> device;
    int         interface_index;
...
typedef void(usbhost_uvc_frame_callback_t)(struct librealuvc::frame_object *frame, void *user_ptr);
...
int usbhost_SendControl(std::shared_ptr<librealuvc::usb_host::device> ihandle,int interface_number, int requestType, int request, int value, int index, unsigned char *buffer, int buflen);
