...
struct usbhost_device *usb_device_open(const char *dev_name)
{
    ...
    struct usbhost_device* result = usb_device_new(dev_name, fd);
    if (result)
        result->writeable = writeable;
    return result;
}

void usb_device_close(struct usbhost_device *device)
{
    close(device->fd);
    free(device);
}

struct usbhost_device *usb_device_new(const char *dev_name, int fd)
{
    struct usbhost_device *device = calloc(1, sizeof(struct usbhost_device));
    int length;
...
static int usb_device_reopen_writeable(struct usbhost_device *device)
{
    if (device->writeable)
        return 1;
...
int usb_device_get_fd(struct usbhost_device *device)
{
...
const char* usb_device_get_name(struct usbhost_device *device)
{
    return device->dev_name;
}

int usb_device_get_unique_id(struct usbhost_device *device)
{
    int bus = 0, dev = 0;
...
uint16_t usb_device_get_vendor_id(struct usbhost_device *device)
{
    struct usb_device_descriptor* desc = (struct usb_device_descriptor*)device->desc;
    return __le16_to_cpu(desc->idVendor);
}

uint16_t usb_device_get_product_id(struct usbhost_device *device)
{
    struct usb_device_descriptor* desc = (struct usb_device_descriptor*)device->desc;
    return __le16_to_cpu(desc->idProduct);
}

const struct usb_device_descriptor* usb_device_get_device_descriptor(struct usbhost_device* device) {
    return (struct usb_device_descriptor*)device->desc;
}

size_t usb_device_get_descriptors_length(const struct usbhost_device* device) {
    return device->desc_length;
}

const unsigned char* usb_device_get_raw_descriptors(const struct usbhost_device* device) {
    return device->desc;
}
 ...
 */
int usb_device_get_string_ucs2(struct usbhost_device* device, int id, int timeout, void** ucs2_out,
 ...
 */
char* usb_device_get_string(struct usbhost_device* device, int id, int timeout) {
...
char* usb_device_get_manufacturer_name(struct usbhost_device *device, int timeout)
{
    struct usb_device_descriptor *desc = (struct usb_device_descriptor *)device->desc;
    return usb_device_get_string(device, desc->iManufacturer, timeout);
}

char* usb_device_get_product_name(struct usbhost_device *device, int timeout)
{
    struct usb_device_descriptor *desc = (struct usb_device_descriptor *)device->desc;
    return usb_device_get_string(device, desc->iProduct, timeout);
}

int usb_device_get_version(struct usbhost_device *device)
{
    struct usb_device_descriptor *desc = (struct usb_device_descriptor *)device->desc;
    return desc->bcdUSB;
}

char* usb_device_get_serial(struct usbhost_device *device, int timeout)
{
    struct usb_device_descriptor *desc = (struct usb_device_descriptor *)device->desc;
    return usb_device_get_string(device, desc->iSerialNumber, timeout);
}

int usb_device_is_writeable(struct usbhost_device *device)
{
    return device->writeable;
}

void usb_descriptor_iter_init(struct usbhost_device *device, struct usb_descriptor_iter *iter)
{
...
int usb_device_claim_interface(struct usbhost_device *device, unsigned int interface)
{
    return ioctl(device->fd, USBDEVFS_CLAIMINTERFACE, &interface);
}

int usb_device_release_interface(struct usbhost_device *device, unsigned int interface)
{
    return ioctl(device->fd, USBDEVFS_RELEASEINTERFACE, &interface);
}

int usb_device_connect_kernel_driver(struct usbhost_device *device,
...
int usb_device_set_configuration(struct usbhost_device *device, int configuration)
{
    return ioctl(device->fd, USBDEVFS_SETCONFIGURATION, &configuration);
}

int usb_device_set_interface(struct usbhost_device *device, unsigned int interface,
...
int usb_device_control_transfer(struct usbhost_device *device,
                                int requestType,
                                int request,
                                int value,
                                int index,
...
int usb_device_bulk_transfer(struct usbhost_device *device,
                             int endpoint,
                             void* buffer,
                             unsigned int length,
                             unsigned int timeout)
...
int usb_device_reset(struct usbhost_device *device)
{
    return ioctl(device->fd, USBDEVFS_RESET);
}

struct usb_request *usb_request_new(struct usbhost_device *dev,
...
struct usb_request *usb_request_wait(struct usbhost_device *dev, int timeoutMillis)
{
